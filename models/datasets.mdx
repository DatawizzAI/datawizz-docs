Looking at the merged PRs, I can see several changes that affect the datasets documentation:

1. **PR #137**: Added validation requiring dataset names (rejecting empty/whitespace names)
2. **PR #133**: Added support for multiple file uploads with cross-file validation
3. **PR #128**: Added DPO training method support which affects how datasets are used for training

These changes directly impact the dataset creation and import functionality described in the documentation.
...
To create a dataset, navigate to the `Datasets` tab in the Datawizz dashboard and click on the `Create Dataset` button. You must provide a name for your new dataset (names cannot be empty or contain only whitespace) and optionally add a description to help you remember what it's for.
...
You can import logs from CSV files by clicking on `Import CSV` and selecting one or more files to upload. Multiple files can be uploaded simultaneously as long as they share the same format and structure. Each file must include `input` and `output` columns, where `input` is the message(s) sent to the model and `output` is the response.
...
When uploading multiple files, Datawizz will validate that all files have compatible formats and schemas before processing them together.
