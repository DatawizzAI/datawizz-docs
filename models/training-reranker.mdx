...
<Note>
`score` and `weight` are **required** fields. For reranker feedback where you're mainly providing corrected scores in `improvement`, use `score: 0` (neutral) and `weight: 1.0`. See [Feedback Signals API](/api-reference/endpoint/post_feedback) for full schema details.
</Note>
