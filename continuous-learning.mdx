...
User feedback can be collected via your application and sent to Datawizz using our API. We recommend capturing **explicit feedback** (e.g. user ratings) as well as **implicit feedback** (e.g. user behavior signals like re-tries, time spent, etc).
...
Datawizz helps you further enhance model performance and safety in production using guardrails, plugins, and routing mechanisms.
...
For model fine tuning - Datawizz lets you re-train models on fresh data as it comes in. Simply create new fine-tuning datasets from the latest logs and feedback, and launch new fine-tuning jobs using older models as the base, to continuously adapt to new data.
